...
export class DoubaoProvider extends AbstractProviderAdapter {
    preferredAnswerSelectors = [
        ".flow-markdown-body",
        ".markdown",
        ".message-content"
    ];
    inputChunkPauseMs = 500;
    inputChunkTargetLength = 120;
    quietPeriodMs = 3000; // 原 5000ms → 1500ms → 3000ms，给页面动画足够安静时间
    loginMarkers = [
        "\u767b\u5f55\u4ee5\u89e3\u9501\u66f4\u591a\u529f\u80fd",
        "\u6296\u97f3\u4e00\u952e\u767b\u5f55",
        "\u8bf7\u5b8c\u6210\u9a8c\u8bc1\u540e\u7ee7\u7eed"
    ...
    async waitForAnswerComplete(ctx) {
        const session = this.getSession(ctx);
        ...
        while (Date.now() < deadline) {
            ...
            if (await this.isStillGenerating(ctx)) {
                if (latestText !== previousText) {
                    ...
                    generatingStableCount = 0;
                } else {
                    generatingStableCount += 1;
                }
                // 连续 10 次（约 3 秒）仍在生成但文本未变化 → 选择器假阳性，放行进入安静期判断
                if (generatingStableCount < 10) {
                    await sleep(300);
                    continue;
                }
            }
    ...
    async extractAnswer(ctx, prompt) {
    ...
    async readLatestAnswerText(ctx) {
        const session = this.getSession(ctx);
    ...
    async isStillGenerating(ctx) {
        ...
        const results = await Promise.all(generatingSelectors.map((sel) => session.page.locator(sel).first().isVisible().catch(() => false)));
