...
export class ProviderWorker {
  ...
  private readonly refreshRetryProviders = new Set(["gemini", "kimi"] as const);
  ...
  private async runWithRefreshRetry(
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        return await this.executePrompt(currentCtx, prompt, startedAt);
      } catch (error) {
  ...
  private shouldRetryAfterRefresh(providerId: ProviderContext["providerId"], message: string): boolean {
    if (!this.refreshRetryProviders.has(providerId as "gemini" | "kimi")) {
      return false;
    }
